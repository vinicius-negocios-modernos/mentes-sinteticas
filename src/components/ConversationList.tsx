...
export default function ConversationList({
  ...
  function handleSelect(conversationId: string) {
    router.push(`/chat/${encodeURIComponent(mindId)}?conversation=${conversationId}`);
  }
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      <button
        ...
        className="w-full px-3 py-2 text-sm rounded-lg bg-purple-600/20 border border-purple-500/30 hover:bg-purple-600/40 text-purple-200 transition-colors text-left"
      >
        + Nova Conversa
      </button>
      ...
      {conversations.map((conv) => (
        <button
          ...
          className={`group w-full px-3 py-2 text-left text-sm rounded-lg border transition-colors ${
